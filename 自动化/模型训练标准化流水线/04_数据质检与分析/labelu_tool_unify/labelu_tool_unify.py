...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="按 inner_id 把来源样本的 rectTool/tagTool 复制到目标范围。")
    parser.add_argument("--config", default=str(DEFAULT_CONFIG_PATH), help="配置文件路径")
    parser.add_argument("--base-url", help="LabelU 服务地址")
    parser.add_argument("--task-id", type=int, help="任务 ID")
    parser.add_argument("--source-inner-id", type=int, help="来源 inner_id")
    ...
    parser.add_argument("--mode", choices=sorted(VALID_MODES), help="复制模式：both / rect / tag，默认 both")
    parser.add_argument("--token", help="访问 token，不传时读取环境变量 LABELU_TOKEN，再读取配置文件")
    parser.add_argument("--rect-tool-name", help="标框工具名，默认 recttool")
    parser.add_argument("--tag-tool-name", help="全局标签工具名，默认 tagtool")
    parser.add_argument("--keep-rect-ids", action="store_true", help="保留来源 rectTool.result 里的 id")
    ...
    parser.add_argument("--page-end", type=int, help="分页结束页，包含；不传表示查到找到目标或列表结束")
...
def build_runtime_config(args: argparse.Namespace, config: dict[str, Any]) -> dict[str, Any]:
    ...
    runtime = {
        ...
        "source_inner_id": args.source_inner_id if args.source_inner_id is not None else target_config.get("source_inner_id"),
        "target_inner_id_start": args.start_inner_id if args.start_inner_id is not None else target_config.get("target_inner_id_start"),
        "target_inner_id_end": args.end_inner_id if args.end_inner_id is not None else target_config.get("target_inner_id_end"),
        "mode": args.mode or target_config.get("mode", "both"),
        "token": args.token or os.getenv("LABELU_TOKEN") or config.get("token"),
        "rect_tool_name": args.rect_tool_name or target_config.get("rect_tool_name", "recttool"),
        "tag_tool_name": args.tag_tool_name or target_config.get("tag_tool_name", "tagtool"),
        "regenerate_rect_ids": False if args.keep_rect_ids else bool(target_config.get("regenerate_rect_ids", True)),
        "regenerate_tag_ids": False if args.keep_tag_ids else bool(target_config.get("regenerate_tag_ids", True)),
        ...
        "request_interval_seconds": args.interval if args.interval is not None else request_config.get("interval_seconds", 0.2),
        "request_timeout_seconds": args.timeout if args.timeout is not None else request_config.get("timeout_seconds", 15),
    ...
    required_fields = ["base_url", "task_id", "source_inner_id", "target_inner_id_start", "target_inner_id_end", "token"]
    ...
    if runtime["mode"] not in VALID_MODES:
        raise ValueError("mode 必须是 both / rect / tag")
    ...
    runtime["headers"] = {
        ...
        "User-Agent": "Mozilla/5.0 LabelU tool unify script",
    }
    runtime["rect_tool_name_normalized"] = runtime["rect_tool_name"].lower()
...
def clone_tool(source_tool: dict[str, Any], regenerate_ids: bool) -> dict[str, Any]:
    tool = copy.deepcopy(source_tool)
    tool.pop("_source_annotation", None)
...
def source_tools(runtime: dict[str, Any], source_annotation: dict[str, Any]) -> dict[str, dict[str, Any]]:
    tools: dict[str, dict[str, Any]] = {}
    if runtime["mode"] in ("both", "rect"):
        rect_key = find_tool_key(source_annotation, runtime["rect_tool_name_normalized"])
        if rect_key is None:
            raise RuntimeError(f"来源 inner_id={runtime['source_inner_id']} 样本没有 rectTool")
        tools["rect"] = source_annotation[rect_key]
        tools["rect"]["_source_annotation"] = {
            key: source_annotation[key] for key in ("width", "height", "rotate") if key in source_annotation
        }
    if runtime["mode"] in ("both", "tag"):
        tag_key = find_tool_key(source_annotation, runtime["tag_tool_name_normalized"])
        if tag_key is None:
            raise RuntimeError(f"来源 inner_id={runtime['source_inner_id']} 样本没有 tagTool")
        tools["tag"] = source_annotation[tag_key]
    return tools
...
def build_patch_payload(
    ...
    if "rect" in tools:
        source_annotation = tools["rect"].get("_source_annotation")
        if isinstance(source_annotation, dict):
            for key in ("width", "height", "rotate"):
                if key not in annotation and key in source_annotation:
                    annotation[key] = source_annotation[key]
        annotation.setdefault("width", 1920)
        annotation.setdefault("height", 1080)
        annotation.setdefault("rotate", 0)
        rect_key = find_tool_key(annotation, runtime["rect_tool_name_normalized"]) or "rectTool"
        annotation[rect_key] = clone_tool(tools["rect"], runtime["regenerate_rect_ids"])

    if "tag" in tools:
        tag_key = find_tool_key(annotation, runtime["tag_tool_name_normalized"]) or "tagTool"
        annotation[tag_key] = clone_tool(tools["tag"], runtime["regenerate_tag_ids"])

    rect_count = get_result_count(annotation, runtime["rect_tool_name_normalized"])
    annotated_count = rect_count if rect_count > 0 else int(target_sample.get("annotated_count") or 0)
    ...
    return {"data": patched_data, "annotated_count": annotated_count}, annotation, parse_note


def patch_sample(runtime: dict[str, Any], sample_id: int, payload: dict[str, Any]) -> requests.Response:
    url = f"{runtime['samples_url']}/{sample_id}"
    return api_request(runtime, "PATCH", url, headers=runtime["headers"], params={"sample_id": sample_id}, json=payload)


def resolve_output_dir(runtime: dict[str, Any]) -> Path:
...
def write_report(runtime: dict[str, Any], rows: list[dict[str, Any]]) -> Path:
    ...
    report_path = resolve_output_dir(runtime) / f"task_{runtime['task_id']}_tool_unify_{runtime['mode']}_{timestamp}.csv"
    fieldnames = [
        "mode",
        "task_id",
        "source_inner_id",
...
def write_sample_backup(runtime: dict[str, Any], sample: dict[str, Any]) -> Path:
    ...
    backup_path = resolve_output_dir(runtime) / f"task_{runtime['task_id']}_tool_unify_{runtime['mode']}_backup_{timestamp}.jsonl"
...
def main() -> int:
    ...
    run_mode = "APPLY" if runtime["apply"] else "DRY-RUN"
    print("当前运行配置:")
    print(f"- 模式: {run_mode}")
    print(f"- 复制模式: {runtime['mode']}")
    ...
    print(f"- 查找页码范围: {runtime['page_start']} - {runtime['page_end'] if runtime['page_end'] is not None else '全部'}")
    print(f"- 标框工具名: {runtime['rect_tool_name']}")
    ...
    try:
        ...
        tools = source_tools(runtime, source_annotation)
        print(f"已读取来源样本 sample_id={source_sample.get('id')}")
        if "rect" in tools:
            print(f"- 来源 rectTool 标框数: {len(tools['rect'].get('result') or [])}")
        if "tag" in tools:
            print(f"- 来源 tagTool 标签数: {len(tools['tag'].get('result') or [])}")

        for index_sample in target_index_samples:
            target_sample = fetch_sample_by_id(runtime, index_sample["id"])
            payload, patched_annotation, parse_note = build_patch_payload(runtime, target_sample, tools)
            rect_count = get_result_count(patched_annotation, runtime["rect_tool_name_normalized"])

...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="把 LabelU 某个 inner_id 样本的 rectTool 复制到一个 inner_id 范围。")
    ...
    parser.add_argument("--source-inner-id", type=int, help="复制 rectTool 的来源 inner_id")
...
def build_runtime_config(args: argparse.Namespace, config: dict[str, Any]) -> dict[str, Any]:
    ...
    runtime = {
        ...
        "regenerate_rect_ids": (
            False if args.keep_rect_ids else bool(target_config.get("regenerate_rect_ids", True))
        ),
    ...
    runtime["headers"] = {
        ...
        "User-Agent": "Mozilla/5.0 LabelU rectTool unify script",
    }
...
def clone_rect_tool(source_rect_tool: dict[str, Any], regenerate_ids: bool) -> dict[str, Any]:
    rect_tool = copy.deepcopy(source_rect_tool)
    rect_tool.pop("_source_annotation", None)
    result = rect_tool.get("result")
...
def build_patch_payload(
    ...
    source_rect_tool: dict[str, Any],
) -> tuple[dict[str, Any], dict[str, Any], str]:
    annotation, parse_note = parse_annotation_result(target_sample)
    source_annotation = source_rect_tool.get("_source_annotation")
    ...
    existing_key = find_tool_key(annotation, runtime["rect_tool_name_normalized"])
    output_key = existing_key or "rectTool"
    annotation[output_key] = clone_rect_tool(source_rect_tool, runtime["regenerate_rect_ids"])

    annotated_count = get_result_count(annotation, runtime["rect_tool_name_normalized"])
...
def write_report(runtime: dict[str, Any], rows: list[dict[str, Any]]) -> Path:
    ...
    report_path = output_dir / f"task_{runtime['task_id']}_recttool_unify_{timestamp}.csv"
...
def write_sample_backup(runtime: dict[str, Any], sample: dict[str, Any]) -> Path:
    ...
    if not output_dir.is_absolute():
        output_dir = Path(__file__).parent / output_dir
    output_dir.mkdir(parents=True, exist_ok=True)
    backup_path = output_dir / f"task_{runtime['task_id']}_recttool_unify_backup_{timestamp}.jsonl"
...
def main() -> int:
    ...
    print(f"- 重新生成标框 id: {runtime['regenerate_rect_ids']}")
    print("- 标记状态: 不改动")
    ...
    try:
        ...
        source_rect_key = find_tool_key(source_annotation, runtime["rect_tool_name_normalized"])
        if source_rect_key is None:
            raise RuntimeError(f"来源 inner_id={runtime['source_inner_id']} 样本没有 rectTool")
        source_rect_tool = source_annotation[source_rect_key]
        source_rect_tool["_source_annotation"] = {
            key: source_annotation[key] for key in ("width", "height", "rotate") if key in source_annotation
        }
        source_rect_count = len(source_rect_tool.get("result", [])) if isinstance(source_rect_tool.get("result"), list) else 0
        print(f"已读取来源样本 sample_id={source_sample.get('id')}，rectTool 标框数={source_rect_count}")

        for index_sample in target_index_samples:
            target_sample = fetch_sample_by_id(runtime, index_sample["id"])
            payload, patched_annotation, parse_note = build_patch_payload(runtime, target_sample, source_rect_tool)
            rect_count = get_result_count(patched_annotation, runtime["rect_tool_name_normalized"])
            tag_count = get_result_count(patched_annotation, "tagtool")
            row = {
                "mode": mode,
